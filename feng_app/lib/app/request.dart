...
import 'package:fengapp/utility/tian_yue_cache_manager.dart';
...
class CacheInterceptor extends InterceptorsWrapper {
  ...
  onRequest(RequestOptions options) async {
    FileInfo info =
        await TianYueCacheManager().getFileFromCache(options.uri.toString());
    if (info != null) {
  ...
  onResponse(Response response) async {
    ...
    var data = response.data;
    await TianYueCacheManager()
        .putStringFile(response.request.uri.toString(), data);
    print("网络数据：" + data);
    return response;
  }
